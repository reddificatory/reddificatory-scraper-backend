...
def store_comment(comment_id, submission_id):
    db.database.cursor.execute(f"INSERT INTO comments (comment_id, submission_id) VALUES ('{comment_id}', '{submission_id}') ON CONFLICT DO NOTHING;")    
...
def get_random_comment(submission_id):
    ...
    comment_ids = get_unused_comments_from_submission(submission_id)
